 ...
 *********************************************/
 ...
 {int} Occupied_Spectrum[Lightpaths] = ...;
 ...
 subject to
 {
    ...
    //modified for no splitting	
 	...
 	//modified for no splitting	
 	forall(<i,j,0> in Lightpaths)
 	  sum(a in Modulations, <x,y> in Split_Lps)a*theta_split[<i,j,0>][<x,y>][a] == Mod_lightpath[<i,j,0>];
 	
 	//Eq(31)
 	//modified for no splitting  
 	forall(<i,j,0> in Lightpaths)
 	  sum(f in 1..slot_number)X_split[f][<i,j,0>][<i,j>] == Width_lightpath[<i,j,0>];
 	
 	forall(<i,j,0> in Lightpaths, <x,y> in Links diff {<i,j>})
 	  sum(f in 1..slot_number)X_split[f][<i,j,0>][<x,y>] == 0;
 	
 	forall(<i,j,0> in Lightpaths, f in Occupied_Spectrum[<i,j,0>])
 	  X_split[f][<i,j,0>][<i,j>] == 1;
    ...
    forall(<ns,nd,nbw> in New_Requests)
    {
 	...
 	forall(<i,j,1> in New_Lightpaths)
 	  ...
 	  sum(<ns,nd,nbw> in New_Requests, band in Bandwidth)band*times*per[band][<ns,nd,nbw>][<i,j,1>] + Occupied_Lightpath_Capacity[<i,j,0>] <= spectrum_size*(Width_lightpath[<i,j,0>]*Mod_lightpath[<i,j,0>] + sum(f in 1..slot_number, a in Modulations)a*rho[<i,j,1>][f][a]);
